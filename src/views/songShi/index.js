...
import { songShi,favour } from "../../api/songShi.js"
...
export default class SongCi extends React.Component{
  ...
  getList(pageNum = 1){
    ...
    songShi({pageNum,keyword}).then(r=>{
  ...
  render() {
    return (
      <div>
        <Poetry title="宋诗" list={this.state.data.list} favour={(id,index) => {this.clickFavour(id,index)}}/>
        <Paginate
